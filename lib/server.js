...
Server.prototype.allowWebsocketConnection = function (req, next) {
  ...
  if (_.isUndefined(req.headers) || _.isUndefined(req.headers.authorization)) {
  ...
  if (!req.headers.authorization.match(/^RC [^\s;]+;[^\s;]+;.+$/)) {
    // Invalid authorization header
    error('invalid format for authorization header: ' + req.headers.authorization);
    next(2, false);
    return;
  }

  var sigArray = req.headers.authorization.replace('RC ', '').split(';', 3);

  // load public key specified by name in sigArray[0]
  fs.readFile(path.join(this.options.certDir, sigArray[0]), {encoding: 'ascii'}, function (err, cert) {
    if (err) {
      // requested public key not found
    ...
    try {
      if (!verify.verify(cert, sigArray[2], 'base64')) {
        // Invalid signature
